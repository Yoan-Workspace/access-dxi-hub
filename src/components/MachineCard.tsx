...
const statusMap: Record<Machine["status"], { label: string; color: string; ring: string }> = {
  ok: { label: "OK", color: "text-success", ring: "bg-success" },
  maintenance: { label: "Maintenance", color: "text-maintenance", ring: "bg-maintenance" },
  danger: { label: "Problème", color: "text-danger", ring: "bg-danger" },
};
...
export function MachineCard({ machine, onEdit }: { machine: Machine; onEdit: () => void }) {
  ...
  return (
    <div
      className={cn(
        ...
        "transition-all hover:-translate-y-0.5 hover:border-primary/40 hover:shadow-[0_8px_30px_-12px_rgba(0,0,0,0.12)]",
      )}
    >
      <span
        className={cn(
        ...
        <div className="min-w-0">
          ...
          <div className="mt-1 flex flex-wrap items-center gap-x-3 gap-y-1 text-xs text-muted-foreground">
            <span className="inline-flex items-center gap-1">
              <MapPin className="h-3 w-3" /> {machine.localisation}
            </span>
            <span className="inline-flex items-center gap-1">
              <Calendar className="h-3 w-3" /> {fmtDate(machine.lastDate)}
            </span>
      ...
      <div className="grid grid-cols-4 gap-2">
        <Counter icon={<Flag className="h-3.5 w-3.5" />} pending={flagsP} total={machine.flags.length} label="Flags" tone="warning" />
        <Counter icon={<XCircle className="h-3.5 w-3.5" />} pending={probsP} total={machine.problems.length} label="Problèmes" tone="danger" />
        <Counter icon={<AlertTriangle className="h-3.5 w-3.5" />} pending={repairsP} total={machine.repairs.length} label="Réparations" tone="warning" />
        <Counter icon={<Lightbulb className="h-3.5 w-3.5" />} pending={improvP} total={machine.improvements.length} label="Improv." tone="improve" />
      </div>
    </div>
  );
}
...
function Counter({
  ...
  return (
    <div className="rounded-lg border bg-background/40 px-2 py-1.5">
      <div className={cn("flex items-center gap-1 text-[11px] font-medium", toneCls)}>
        {icon}
        <span>{label}</span>
      </div>
      <div className="mt-0.5 text-sm font-semibold tabular-nums">
        <span className={cn(active ? toneCls : "text-foreground")}>{pending}</span>
